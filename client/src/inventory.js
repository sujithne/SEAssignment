...
const InventoryList = () => {
    ...
    useEffect(() => {
        axios.get('http://18.210.66.126:3001:3001/inventory')
            .then(res => setInventory(res.data))
            .catch(err => console.log(err));
    }, []);

    const handleDelete = (id) => {
        axios.delete(`http://18.210.66.126:3001/inventorys/${id}`)
            .then(() => {
...
const handleEdit = (id) => {
    ...
    // setName(id.name);
    //setQty(qty);
    axios.get(`http://18.210.66.126:3001/inventorys/${id}`).then((response) => {
        setIid(id);
...
const updateItem = () => {
    axios.put(`http://18.210.66.126:3001/inventorys/${iid}`, {
        name,
        quantity,
